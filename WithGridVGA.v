...
module binary_loader(
    ...
    //Game board mapping (10x20 blocks, each 24x24 pixels)
    ...
    // Column & Row calculation (via case statements)
    always @(posedge clk) begin
        // Defaults (invalid outside grid)
        block_col <= 4'd15;
        block_row <= 5'd31;
    ...
    // Pixel output with grid overlay
    always @(posedge clk) begin
        if (h_count < H_ACTIVE && v_count < V_ACTIVE) begin
            if ((h_count >= START_X) && (h_count < START_X+BOARD_WIDTH) &&
                (v_count >= START_Y) && (v_count < START_Y+BOARD_HEIGHT)) begin

                // Grid lines (1-pixel wide, always white)
                if ( ((h_count - START_X) % BLOCK_SIZE == 0) ||
                     ((v_count - START_Y) % BLOCK_SIZE == 0) )
                    out <= 1;
                else if (block_index < 200)
