module simple_tetris(
        ...
        begin
            ...
            for (kk = 0; kk < 16; kk = kk + 1) begin
                if (tetrominos[piece_idx][rot_idx][15 - kk]) begin
                    ...
                    // Only check collision with settled blocks that are ON the visible board
                    if (row_tmp >= 0 && row_tmp < ROWS && col_tmp >= 0 && col_tmp < COLS) begin
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            // If game over, freeze the game (no further state changes)
            if (game_over) begin
                // still update debounced inputs so outputs won't glitch if desired,
                // but don't progress state machine or place new pieces.
                cmd_prev <= cmd;
                cmd_edge <= cmd & ~cmd_prev;
            end else begin
                ...
                case (state)
                    0: begin // spawn new piece
                        // Check for game over - only check collision with settled blocks on board
                        if (spawn_collision(seq_index, 2'd0, 6'd3, 6'd0)) begin
                            // Game over - piece cannot spawn without collision
                            game_over <= 1;
                        end else begin
